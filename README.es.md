...
Un script en Bash que automatiza la descarga e instalación de Splunk en un sistema Linux basado en Debian/Ubuntu (usa `dpkg` y `apt`).
...
4. Si `dpkg` falla por dependencias faltantes, intenta corregirlas automáticamente.
...
## Problema conocido: bloqueo (lock) de dpkg
